...
public class UpdateJoinEvent implements Listener
{
    ...
    public void onJoin(final PlayerJoinEvent e) {
    	...
    	if (p.hasPermission("noweather.update")) {
    		if (MainNoRain.getPlugin().getConfig().getBoolean("Update.Enabled") == true){
    		this.checker = new UpdateChecker(MainNoRain.plugin);
                        if (this.checker.isConnected()) {
                            if (this.checker.hasUpdate()) {
                                ...
                                p.sendMessage(ChatColor.RED + "Your version: " + MainNoRain.plugin.getDescription().getVersion());
                                p.sendMessage(ChatColor.GRAY + "=========================");
                            }
                        }               
       }
